...
const ScanTab = () => {
  ...
  const handleSnap = async () => {
    if (webcamRef.current) {
      ...
      // Batch mode: enqueue and process without opening result popup; no UI animation
      if (scanMode === 'batch') {
      ...
      <div className="relative">
        <div className="bg-gray-800 rounded-2xl border-2 border-gray-700 overflow-hidden aspect-square max-w-lg mx-auto relative">
          ...
          <div className="w-full h-full bg-gradient-to-br from-gray-900 to-gray-800 flex items-center justify-center relative">
            ...
            {/* Spinner only for individual mode; no snap animation in batch */}
            ...
            <div className="grid gap-4 p-6 md:grid-cols-3">
              ...
              <div className="rounded-xl border border-red-600/40 bg-red-600/10 p-4 min-w-0">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-red-300">{contaminatedImages.length}</span>
                </div>
                <div className="mt-3">
                  {contaminatedImages.length === 0 ? (
                    <p className="text-xs text-red-200/80">No contaminated detections yet.</p>
                  ) : (
                    <div className="overflow-x-auto scrollbar-hide w-full">
                      <div className="flex gap-2" style={{ width: 'max-content' }}>
                        {Array.from({ length: Math.ceil(contaminatedImages.length / 2) }).map((_, rowIdx) => (
                          <div key={rowIdx} className="flex flex-col gap-2">
                            {contaminatedImages.slice(rowIdx * 2, rowIdx * 2 + 2).map((item, idx) => (
                              <img
                            ...
                            {/* Fill empty slot if odd number */}
                            {contaminatedImages.slice(rowIdx * 2, rowIdx * 2 + 2).length === 1 && (
                              <div className="w-20 h-20 flex-shrink-0"></div>
                            )}
                          </div>
                        ))}
