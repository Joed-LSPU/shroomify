    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-2000 p-4">
      <div className="bg-gray-900 rounded-2xl border border-gray-700 max-w-md w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          ...
          {/* Action Buttons */}
          <div className={`flex space-x-3 pt-4 ${isLoggedIn ? '' : 'justify-center'}`}>
            <button
              onClick={onClose}
              className={`${isLoggedIn ? 'flex-1' : 'w-full'} bg-gray-800 hover:bg-gray-700 text-white py-3 px-4 rounded-lg transition-colors font-medium`}
            >
              Close
            </button>
            {isLoggedIn === true && !isSaved && (
              <button
                onClick={async () => {
                  if (result !== null && previewImage && confidence !== null && userEmail) {
                    try {
