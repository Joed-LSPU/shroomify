...
const ProfileTab: React.FC<ProfileTabProps> = ({ 
  ...
  const StatisticsBarGraph = ({ stats, loading }: { stats: typeof statistics, loading: boolean }) => {
    ...
    // Filter out data points with zero values for the bar chart
    const nonZeroDataPoints = dataPoints.filter(item => item.value > 0);
              ...
              <div className="flex items-end justify-between space-x-2 h-32">
                {nonZeroDataPoints.map((item, _index) => (
                  <div key={item.label} className="flex-1 flex flex-col items-center group">
                    <div className="w-full bg-gray-600/50 rounded-t-lg relative h-full flex items-end">
                      <div
                        className={`w-full ${item.color} rounded-t-lg transition-all duration-700 ease-out group-hover:opacity-90`}
                        style={{ 
